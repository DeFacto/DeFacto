 ...
 */
public class PageRankFeature extends AbstractFeature {
    ...
    public void extractFeature(Evidence evidence) {
        ...
        for ( Map.Entry<Pattern, List<WebSite>> patternToWebSites : evidence.getWebSites().entrySet()) {
            for ( WebSite website : patternToWebSites.getValue() ) {
                ...
                if ( website.getPageRank() >= 0 && website.getPageRank() < Constants.UNASSIGNED_PAGE_RANK ) {
...
//                    }
        ...
        evidence.getFeatures().setValue(AbstractFeature.PAGE_RANK_MAX, maxScore);
        evidence.getFeatures().setValue(AbstractFeature.PAGE_RANK_SUM, sumScore);
